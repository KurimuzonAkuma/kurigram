...
async def resolve_raw_media(
    ...
    """Prepare the media to be sent in raw.
    ...
    Returns:
        ...
        | :obj:`~pyrogram.raw.types.InputMediaPhotoExternal`
    ...
    if isinstance(media, types.InputMediaPhoto):  # TODO: ttl_seconds, live_photo
                ...
                raw.functions.messages.UploadMedia(
                    ...
                    media=raw.types.InputMediaUploadedPhoto(
                        file=await client.save_file(
                            media.media, progress=progress, progress_args=progress_args
                        ),
        ...
        return get_input_media_from_file_id(
            media.media,
            FileType.PHOTO,
            has_spoiler=media.has_spoiler,
        )

    if isinstance(media, types.InputMediaVideo):  # TODO: ttl_seconds
        input_video_cover = None

        if media.video_cover is not None:
            if (
                isinstance(media.video_cover, io.BytesIO)
                or pathlib.Path(media.video_cover).is_file()
            ):
                uploaded_media = await client.invoke(
                    raw.functions.messages.UploadMedia(
                        peer=peer,
                ...
                uploaded_media = await client.invoke(
                    raw.functions.messages.UploadMedia(
                        peer=peer, media=raw.types.InputMediaPhotoExternal(url=media.video_cover)
                    )
                )
                ...
                input_video_cover = get_input_media_from_file_id(
                    ...
                    media=raw.types.InputMediaUploadedDocument(
                        ...
                        attributes=[
                            ...
                            raw.types.DocumentAttributeFilename(
                                file_name=media.file_name or pathlib.Path(media.media).name,
                            ),
                        ],
                    ),
                ),
    ...
    if isinstance(media, types.InputMediaAudio):  # TODO: ttl_seconds
        if isinstance(media.media, io.BytesIO) or pathlib.Path(media.media).is_file():
        ...
        return get_input_media_from_file_id(media.media, FileType.AUDIO)

    if isinstance(media, types.InputMediaAnimation):
        if isinstance(media.media, io.BytesIO) or pathlib.Path(media.media).is_file():
            uploaded_media = await client.invoke(
                raw.functions.messages.UploadMedia(
                    peer=peer,
    ...
    if isinstance(media, types.InputMediaDocument):  # TODO: force_document
        if isinstance(media.media, io.BytesIO) or pathlib.Path(media.media).is_file():
            uploaded_media = await client.invoke(
                raw.functions.messages.UploadMedia(
                    peer=peer,
        ...
        return get_input_media_from_file_id(media.media, FileType.DOCUMENT)

    if isinstance(media, types.InputMediaSticker):
        if isinstance(media.media, io.BytesIO) or pathlib.Path(media.media).is_file():
            uploaded_media = await client.invoke(
                raw.functions.messages.UploadMedia(
                    peer=peer,
                    media=raw.types.InputMediaUploadedDocument(
                        mime_type=client.guess_mime_type(media.media) or "image/webp",
                        file=await client.save_file(
                            media.media, progress=progress, progress_args=progress_args
                        ),
                        attributes=[
                            raw.types.DocumentAttributeFilename(
                                file_name=pathlib.Path(media.media).name,
                            ),
                            raw.types.DocumentAttributeSticker(
                                alt=media.emoji, stickerset=raw.types.InputStickerSetEmpty()
                            ),
                        ],
                    ),
                ),
        ...
        return get_input_media_from_file_id(media.media, FileType.STICKER)

    if isinstance(media, types.Location):
        if media.live_period is not None:
            return raw.types.InputMediaGeoLive(
                geo_point=raw.types.InputGeoPoint(
                    lat=media.latitude or 0,
                    long=media.longitude or 0,
                    accuracy_radius=media.accuracy_radius,
                ),
                heading=media.heading,
                period=media.live_period,
                proximity_notification_radius=media.proximity_alert_radius,
            )
